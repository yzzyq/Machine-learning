...
#完整的SMO的运行
#这是对之前那个SMO进行优化
#选取第一个alphas的时候，单遍扫描和非边界实现单遍扫描混合的
#选取第二个alphas的时候，采用最大化步长的方式选择第二个alphas
...
def innerSmoTrain(i,svm):
    ...
    #print('errorFirst:',errorFirst)
    #print(svm.alphas[i] < svm.C)
    if ((svm.labelSet[0,i]*errorFirst < -svm.toler) and (svm.alphas[i] < svm.C)) \
       or ((svm.labelSet[0,i]*errorFirst > svm.toler) and (svm.alphas[i] < 0)):
        ...
        if eta > 0:
            return 0
        #print('更新：',svm.labelSet[0,second]*(errorFirst - secondError))
        #print('eta:',eta)
